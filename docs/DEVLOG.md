...
## 2026-09-17 | 观察仓字体二次放大

- **执行者**：Codex App
- **任务**：将观察仓核心行情字体放大至接近此前两倍
- **修改文件**：`public/styles.css`
- **布局**：单格改为两行大字布局；桌面/平板每屏显示 2 格，手机每屏显示 1 格，继续左右滑动查看十个位置
- **数据安全**：仅修改展示样式，保持观察仓 35% 高度且未修改 `watchlist` 真源内容
- **验收**：正常屏名称 46px、涨幅 58px、现价 32px、MA 24px；四种测试尺寸均无页面或单元格溢出，可滑到第 10 格

---

## 2026-09-17 | 观察仓新增云南锗业与雅克科技

- **执行者**：Codex App
- **任务**：按用户指令向观察仓追加云南锗业和雅克科技
- **证券信息**：云南锗业 `002428`；雅克科技 `002409`
- **数据安全**：仅追加两只观察标的，原有 5 只观察标的和实际持仓字段均保持不变
- **真源与快照**：已更新 LaunchAgent 使用的 OpenClaw 真源，并在写入前生成独立快照；仓库内兜底文件同步更新
- **验收**：真源与 `/api/raw-holdings` 均为 7 只，页面显示 `7/10`；两只新增标的实时行情与 MA 数据正常；`npm test` 40 pass / 0 fail

---

## 2026-09-17 | 观察仓字号回调 30%

- **执行者**：Codex App
- **任务**：观察仓大字版缩小约 30%，提升同屏信息密度
- **布局**：宽屏每屏 4 格、平板 3 格、手机 2 格，继续左右滑动查看十个位置
- **数据安全**：仅修改 CSS，观察仓 35% 高度及 `watchlist` 真源内容均未改变
- **验收**：1280×720、800×720、800×480、390×844、390×640 均达到目标同屏数量，无单元格或页面溢出

---

## 2026-09-17 | 屏幕 3 观察仓可读性修复

- **执行者**：Codex App
- **现场验证**：直接检查 Safari 全屏看板；观察仓实际为横向 4 列，并非上下两排
- **根因**：浏览器工具栏使网页高度低于 700px，误触发矮屏缩字规则，现价与 MA 分别缩至 15px、12px
- **修复**：矮屏断点收紧至 520px；屏幕 3 保持横向 4 列；卡片内名称、涨幅、现价、均线各占一行，现价提升至 24px、MA 提升至 20px
- **数据安全**：仅修改 CSS 断点，未修改任何持仓或观察仓数据
- **验收**：在屏幕 3 的 Safari 全屏窗口直接刷新并目视确认；另验证 1280×668、800×720、800×480、390×844、390×640，均无卡片或页面溢出
